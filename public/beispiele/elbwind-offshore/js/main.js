/* ==========================================================================
   Elbwind Offshore — Premium tier demo
   Scroll reveals, fixed header state, animated counters (German decimal
   comma), mobile nav, contact form validation (simulated submit).
   The canvas hero lives in hero-wind.js and loads on the home page only.
   ========================================================================== */
...
(function () {
  ...
  if (toggle && nav) {
    ...
    // 24/7 footer for the Leitwarte menu (hidden on desktop via CSS)
    nav.insertAdjacentHTML(
      "beforeend",
      '<div class="main-nav__meta">' +
        "53,4712° N · 9,9797° O — Harburg<br>" +
        'Leitwarte 24/7: <a href="tel:+494055589299">+49 40 555 892 99</a>' +
      "</div>"
    );
  ...
  /* ---------- Live wind readout in the hero (home page only) ---------- */
  var windline = document.querySelector("[data-windline]");
  if (windline && !prefersReducedMotion) {
    var readings = [
      "SW 5 Bft · 19 kn",
      "SW 6 Bft · 24 kn",
      "W 6 Bft · Böen 31 kn",
    ...
    setInterval(function () {
      windline.classList.add("is-swapping");
      setTimeout(function () {
        idx = (idx + 1) % readings.length;
        windline.textContent = readings[idx];
        windline.classList.remove("is-swapping");
      }, 350);
    }, 3400);
  }
  ...
  if (counters.length) {
    var animateCounter = function (el) {
      ...
      var isDecimal = el.getAttribute("data-decimal") === "true"; // 976 -> "97,6"
      var group = el.getAttribute("data-group") === "true";       // 17500 -> "17.500"
  ...
  if (form) {
    ...
    var rules = [
      ...
      },
      {
        ...
        validate: function (el) {
          return el.value ? "" : "Bitte wählen Sie den Leistungsbereich aus.";
        }
      },
      {
